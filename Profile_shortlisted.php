...
<main class="mvv-page">
  <section class="mvv-page-hero">
    <div class="mvv-container">
      ...
      <h1>Shortlisted By Me</h1>
      <p>Profiles you have shortlisted</p>
      <nav class="mvv-breadcrumb" aria-label="breadcrumb">
        <a href="index_dashboard">Home</a>
        <span>Shortlisted By Me</span>
      </nav>
    </div>
  </section>

<?php
$exclSql = $excl ? " AND profile_id NOT IN($excl)" : '';
$sqlview = mysqli_query($con,"SELECT * FROM shortlist_profile where mat_id='$login' $exclSql and banstatus!='1' LIMIT $start_from, $limit");
$sql1="SELECT COUNT(*) as totalCount FROM shortlist_profile where mat_id='$login' $exclSql and banstatus!='1'";
  ...
  <div class="mvv-container">
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="Profile_shortlisted?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="Profile_shortlisted?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="Profile_shortlisted?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
...
<section class="mvv-section">
  <div class="mvv-container">
    <div style="text-align:center;padding:80px 20px;">
      ...
      <p style="color:#999;">Not yet shortlisted any profiles.</p>
