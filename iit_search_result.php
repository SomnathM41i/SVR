...
if(isset($_GET["page"]))
	...
	$pageLimit = ($page * $setLimit) - $setLimit;
	 ...
	 if(isset($_POST['iit']))
	 $iit1=implode("','",$_POST['iit']);
	 $explode_iit=explode("a:1:{i:0;s:",$_GET['iit']);
	 $explode_iit1=explode(":",$iit[1]);
	 $trim_iit=trim($explode_iit1[1],'"');
	 $rtrim_iit=rtrim($trim_iit,'";}');
	 $iit=$iit1 ? $iit1 : $rtrim_iit;
	 $iit123=array($iit);
...
if($iit!="Any" and $iit!="")
{
	$sql=$sql." and instu IN('".$iit."') and iit = 'Yes' ";
}
$sql=$sql."and iit = 'Yes' ";
...
if(isset($_POST['iit']))
	 $iit1=implode("','",$_POST['iit']);
 	 $explode_iit=explode("a:1:{i:0;s:",$_GET['iit']);
	 $explode_iit1=explode(":",$explode_iit[1]);
	 $trim_iit=trim($explode_iit1[1],'"');
	 $rtrim_iit=rtrim($trim_iit,'";}');
	 $iit=$iit1 ? $iit1 : $rtrim_iit;
	 $iit123=array($iit);
	 $iiturl=urlencode(serialize($iit123));
...
$profile=implode("','", $data);
	   ...
	   $sql1 = "SELECT COUNT(*) as totalCount from register where ";
		...
		if($iit!="Any" and $iit!="")
		{
			$sql1=$sql1." and instu IN('".$iit."') AND iit = 'Yes' ";
		}
		$sql1=$sql1." AND iit = 'Yes' ";
    	...
    	{	
    			...
    			for ($counter = 1; $counter <= $setLastpage; $counter++)
    			{
    				if ($counter == $page)
    					$setPaginate.= "<li class='active'><span>$counter</span></li>";
     					...
     					$setPaginate.= "<li><a href='{$page_url}page=$counter&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>$counter</a></li>";					
       			}
          	}
    		elseif($setLastpage > 5 + ($adjacents * 2))
    		{
    			if($page < 1 + ($adjacents * 2))		
    			{
    				...
    				$setPaginate.= "<li><a href='{$page_url}page=$lpm1&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>$lpm1</a></li>";
    				$setPaginate.= "<li><a href='{$page_url}page=$setLastpage&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>$setLastpage</a></li>";		
				}
    			elseif($setLastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
    			 {
    				$setPaginate.= "<li><a href='{$page_url}page=1&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>1</a></li>";
    				$setPaginate.= "<li><a href='{$page_url}page=2&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>2</a></li>";
    				$setPaginate.= "<li class='mvv-dot'>...</li>";
    				for ($counter = $page - $adjacents; $counter <= $page + $adjacents; $counter++)
    					...
    					if ($counter == $page)
    				        ...
    				        $setPaginate.= "<li class='mvv-dot'>..</li>";
    			        	$setPaginate.= "<li><a href='{$page_url}page=$lpm1&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>$lpm1</a></li>";
    				        $setPaginate.= "<li><a href='{$page_url}page=$setLastpage&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>$setLastpage</a></li>";		
    			     }
    		        else
    		      	{
	    				$setPaginate.= "<li><a href='{$page_url}page=1&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'>1</a></li>";
    		...
    		if ($page < $counter - 1){ 
    			$setPaginate.= "<li><a href='{$page_url}page=$next&txtSAge=$from_age&txtEAge=$to_age&religion=$religionurl&iit=$iiturl&edu=$eduurl&occu=occuurl&looking=$lookingurl&gender=$txtgender&with_photo=$with_photo'><b>></b></a></li>";
    		}else{
    			$setPaginate.= "<li class='active'><span><b>></b></span></li>";
            }
...
<main class="mvv-page">
  <section class="mvv-page-hero">
    <div class="mvv-container">
      <div class="mvv-eyebrow">IIT / IIM / NIT</div>
      <h1>Search Result</h1>
      <nav class="mvv-breadcrumb" aria-label="breadcrumb">
        <a href="<?php echo isset($login) ? 'index_dashboard' : 'index'; ?>">Home</a>
        <span>IIT/NIT/IIM Search Results</span>
      </nav>
    </div>
  </section>
  ...
  <section class="mvv-section">
    <div class="mvv-container">
          ...
          <div class="mvv-match-card">
            ...
            <div class="mvv-card-body">
              ...
              <div class="mvv-card-meta">
                ...
                <?php echo $fetch['Age'] ?: 'Not Set'; ?> Yrs, <?php echo $heightMap[$fetch['Height']] ?? 'Not Set'; ?>
              </div>
            </div>
      ...
      <div style="text-align:center;padding:80px 20px;">
        ...
        <p style="color:#999;">No profiles match your search criteria.</p>
        <a href="iit_search" class="mvv-btn primary" style="margin-top:10px;">Search Again</a>
      </div>
      <?php } ?>
    </div>
  </section>
