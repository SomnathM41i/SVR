...
<head>
<meta charset="utf-8">
<title>My Connected Members</title>
...
<main class="mvv-page">
  <section class="mvv-page-hero">
    <div class="mvv-container">
      ...
      <h1>My Connected Members</h1>
      <p>तुमचे कनेक्टेड सदस्य पहा</p>
      <nav class="mvv-breadcrumb" aria-label="breadcrumb">
        <a href="index_dashboard">Home</a>
        <span>My Connected Members</span>
      </nav>
    </div>
  </section>

<?php
$exclSql = $excl ? " AND eireceiver NOT IN($excl)" : '';
$sqlview = mysqli_query($con,"SELECT * FROM expressinterest where eisender='$login' And status='Accept' $exclSql and banstatus!='1' LIMIT $start_from, $limit");
$sql1="SELECT COUNT(*) FROM expressinterest where eisender='$login' And status='Accept' $exclSql and banstatus!='1'";
...
<section class="mvv-section">
  <div class="mvv-container">
    <div class="mvv-activity-grid">
      <?php while($fetch=mysqli_fetch_array($sqlview)) {
        $whom=$fetch['eireceiver'];
        $sqldata=mysqli_query($con,"select * from register where matriid='$whom'");
        $rec=mysqli_fetch_array($sqldata);
        $is_block = mysqli_query($con,"select *from block_member where matriid='$login' AND profile_id='".$fetch['eireceiver']."'");
      ...
      <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12">
        <div class="mvv-match-card">
          ...
          <div class="mvv-card-body">
            ...
            <div style="font-size:0.8rem;color:var(--mvv-muted);margin-top:4px;">Date: <?php echo $fetch['eisentdt'];?></div>
          </div>
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="my_connected_members?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="my_connected_members?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="my_connected_members?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
...
<section class="mvv-section">
  <div class="mvv-container">
    <div style="text-align:center;padding:80px 20px;">
      ...
      <p style="color:#999;">You have not yet connected profiles.</p>
