...
$match_qry = "select DISTINCT * from register where visibility NOT LIKE 'hidden' AND Status NOT LIKE 'Banned' AND MatriID NOT LIKE '$login' AND ";
...
if($me['Looking']!="" && $me['Looking']!="Any") {
  $terms=array_map(function($v){return "'$v'";},explode(" , ",$me['Looking']));
  $match_qry.="Maritalstatus IN(".implode(',',$terms).") AND";
}
$match_qry.=" Gender='$match_sex' AND Height BETWEEN '$pe_from_height'AND'$pe_to_height' AND Age BETWEEN '$pe_fromage' AND '$pe_toage'";
...
function displayPaginationBelow($con,$per_page,$page){
  $login=$_SESSION['MatriID']; $page_url="?";
  ...
  $adjacents="2"; $setLastpage=ceil($total/$per_page); $lpm1=$setLastpage-1;
  $h='';
  if($setLastpage>1){
...
@media (max-width:767.98px){
  .mvv-page{padding-bottom:36px;}
  .mvv-container{padding-left:12px;padding-right:12px;}
  .mvv-match-card .mvv-card-img{height:230px;}
  .mvv-match-card .mvv-card-body{padding:14px;}
  .mvv-match-card .mvv-card-actions{padding:10px 14px;}
  .mvv-match-card .mvv-card-actions a{flex:1;width:auto;min-width:42px;border-radius:8px;}
  .mvv-pagination{gap:4px;margin-top:20px;}
  .mvv-pagination li a,.mvv-pagination li span,.mvv-pagination li.active span{min-width:34px;height:34px;font-size:.82rem;}
  .mvv-pagination .mvv-page-info{width:100%;text-align:center;}
}
@media (max-width:380px){
  .mvv-match-card .mvv-card-img{height:200px;}
  .mvv-match-card .mvv-card-meta{font-size:.8rem;}
}
</style>
</head>
<body>
...
<main class="mvv-page">
  <section class="mvv-page-hero">
    <div class="mvv-container">
      ...
      <p>Based on your partner preferences</p>
  ...
  <section class="mvv-section">
    <div class="mvv-container">
      ...
      $count.=" Gender='$match_sex' AND Height BETWEEN '$pe_from_height'AND'$pe_to_height' AND Age BETWEEN '$pe_fromage' AND '$pe_toage'";
      foreach(['Religion','Complexion','Caste','Residentstatus','Country','State','Occupation','Education'] as $f){
        $vf='PE_'.$f; $dbCol=($f=='Residentstatus')?'Residencystatus':$f;
        if($me[$vf]!=""&&$me[$vf]!="Any"){
          $t=array_map(function($v){return "'$v'";},explode(",",$me[$vf]));
      ...
      <div style="text-align:center;padding:80px 20px;">
        ...
        <p style="color:#999;">You have not any Mutual Matches.</p>
  ...
  <div class="modal-dialog">
    <div class="modal-content" style="border-radius:12px;">
      <div class="modal-header" style="background:var(--mvv-maroon);color:#fff;border-radius:12px 12px 0 0;">
        <button type="button" class="close reminder-modal-close" data-bs-dismiss="modal" aria-label="Close" style="color:#fff;opacity:.8;">&times;</button>
      ...
      <div class="modal-body" style="padding:20px;">
        ...
        <?php $cnt--; }
        $q2f=mysqli_num_rows(mysqli_query($con,"select * from compatibility where MatriID='$login'"));
        if($q2f=='0'){ ?>
        ...
        <?php $cnt--; }
        $q3fn=mysqli_num_rows(mysqli_query($con,"select * from register where MatriID='$login' and Photo1!='nophoto.jpg' and Photo1!=''"));
        if($q3fn=='0'){ ?>
        ...
        if(!($q4f['memtype']!='Free' && $q4f['Noofcontacts']>0 && strtotime($q4f['MemshipExpiryDate'])>strtotime(date('Y-m-d')))){ ?>
          <a href="my_offer" style="color:var(--mvv-maroon);text-decoration:none;display:block;margin-bottom:8px;"><i class="fa fa-credit-card" style="margin-right:8px;"></i> Your Membership Plan status is <b><?php echo $q4f['memtype'] ?></b>. Upgrade it Now.</a>
        <?php $cnt--; }
        if($cnt==4){ ?>
          <div style="color:#28a745;"><i class="fas fa-check-circle"></i> Email Verified</div>
          <div style="color:#28a745;"><i class="fas fa-check-circle"></i> Compatibility Updated Successfully</div>
          <div style="color:#28a745;"><i class="fas fa-check-circle"></i> Profile Photo Updated</div>
          <div style="color:#28a745;"><i class="fas fa-check-circle"></i> Membership Status is Paid</div>
        <?php } ?>
      </div>
      <div class="modal-footer" style="padding:12px 20px;">
        <button type="button" class="mvv-btn primary reminder-modal-close" data-bs-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('click', function(e) {
  if (e.target.closest('.reminder-modal-close')) {
    closeReminderModal();
  }
});
</script>
