...
try {
    ...
    $incomeFrom = annual_income_normalize_value($_POST['income_from'] ?? '');
    $incomeTo = annual_income_normalize_value($_POST['income_to'] ?? '');
    ...
    if (
        ($incomeFrom !== '' && !annual_income_is_valid($incomeFrom, true))
        || ($incomeTo !== '' && !annual_income_is_valid($incomeTo, true))
    ) {
        echo json_encode(["status" => "error", "message" => "Invalid annual income range"]);
        exit;
    }
    if ($incomeFrom !== '' && $incomeTo !== '' && (int)$incomeFrom > (int)$incomeTo) {
        echo json_encode(["status" => "error", "message" => "Minimum annual income cannot be greater than maximum annual income"]);
        exit;
    }
    ...
    if ($district != 'Any' && !empty($district)) {
        $districtStr = "'" . implode("','", array_map(function($dis) use ($con) {
            return mysqli_real_escape_string($con, $dis);
        }, $districts)) . "'";
    ...
    if ($incomeFrom !== '') {
        $sql .= " AND CAST(Annualincome AS UNSIGNED) >= " . (int)$incomeFrom;
    }
    if ($incomeTo !== '') {
        $sql .= " AND CAST(Annualincome AS UNSIGNED) <= " . (int)$incomeTo;
    }
    ...
    if ($incomeFrom !== '') {
        $countSql .= " AND CAST(Annualincome AS UNSIGNED) >= " . (int)$incomeFrom;
    }
    if ($incomeTo !== '') {
        $countSql .= " AND CAST(Annualincome AS UNSIGNED) <= " . (int)$incomeTo;
    }
    if ($with_photo != 'Any' && $with_photo != 'withoutphoto') {
    ...
    if (mysqli_num_rows($rs_result) > 0) {
        ...
        echo json_encode([
            ...
            "search_criteria" => [
                ...
                "annual_income_range" => [$incomeFrom, $incomeTo],
