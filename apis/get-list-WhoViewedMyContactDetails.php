...
/**
 * STEP 2: Count total profiles that viewed my contact excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND who1 NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND who1 NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
...
$countSql = "SELECT COUNT(*) as total 
             FROM viewedaddress 
             WHERE whom1 = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch profiles that viewed my contact with pagination
 */
$sql = "SELECT who1, when1 
        FROM viewedaddress 
        WHERE whom1 = ? AND banstatus != '1' $whereNotIn 
        ORDER BY when1 DESC 
        LIMIT ?, ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch profile details of the user who viewed the contact
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Profiles that viewed your contact number fetched successfully" : "No one has viewed your contact number yet",
    "pagination" => [
        "current_page" => $page,
