...
/**
 * STEP 2: Count total viewed contact profiles excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND whom1 NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND whom1 NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
...
$countSql = "SELECT COUNT(*) as total 
             FROM viewedaddress 
             WHERE who1 = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch viewed contact profiles with pagination
 */
$sql = "SELECT whom1, when1 
        FROM viewedaddress 
        WHERE who1 = ? AND banstatus != '1' $whereNotIn 
        ORDER BY when1 DESC 
        LIMIT ?, ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch profile details of the viewed contact
    $pstmt = $con->prepare("SELECT MatriID, Education, Occupation, Age, Height, Photo1, photo_visibility, Photo1Approve, 
                                   Name, Fathername, Mothersname, ConfirmEmail, Mobile, Mobile2, DOB, POB, TOB, 
                                   Address, Country, State, Dist, City, work_address, working_country, working_state, 
                                   working_dist, working_taluka, working_city, verifymobile, idproof_approve, horoscope, HorosApprove 
                            FROM register WHERE MatriID = ?");
    $pstmt->bind_param("s", $viewedId);
    $pstmt->execute();
    $profileData = $pstmt->get_result()->fetch_assoc();

    if ($profileData) {
        // Fetch email verification
        $estmt = $con->prepare("SELECT verification FROM emailverify WHERE MatriID = ?");
        $estmt->bind_param("s", $viewedId);
        $estmt->execute();
        $emailData = $estmt->get_result()->fetch_assoc();
        $emailVerified = $emailData['verification'] ?? 'No';

        // Fetch document verification
        $dstmt = $con->prepare("SELECT docapprove FROM document WHERE MatriID = ?");
        $dstmt->bind_param("s", $viewedId);
        $dstmt->execute();
        $docData = $dstmt->get_result()->fetch_assoc();
        ...
        // Add horoscope details if available and approved
        if ($profileData['HorosApprove'] == 'Yes' && !empty($profileData['horoscope'])) {
            $profile["ContactDetails"]["Horoscope"] = "kundli/" . $profileData['horoscope'];
        }
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Viewed contact profiles fetched successfully" : "No contact profiles have been viewed yet",
    "pagination" => [
        "current_page" => $page,
