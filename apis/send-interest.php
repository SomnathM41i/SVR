...
try {
    ...
    $eisentdt = date('d-m-Y');
    $status = 'Pending';
    $insertStmt = $con->prepare("INSERT INTO expressinterest (eisender, eireceiver, name, eisentdt, status) 
                                 VALUES (?, ?, ?, ?, ?)");
    $insertStmt->bind_param("sssss", $matriId, $receiverId, $receiverId, $eisentdt, $status);
    ...
    $notiStmt = $con->prepare("INSERT INTO notification 
                               ...
                               VALUES (?, ?, 'Connected', 'Connected Members', 'unseen', NOW())");
    $notiStmt->bind_param("ss", $matriId, $receiverId);
    $notiStmt->execute();

    /**
     * STEP 4: Fetch sender and receiver details for email
     */
    $senderStmt = $con->prepare("SELECT Name, Photo1, Age, Height, Religion, mother_tounge, Education, Occupation, 
                                        ConfirmEmail, City 
                                 FROM register WHERE MatriID = ?");
    $senderStmt->bind_param("s", $matriId);
    $senderStmt->execute();
    $senderData = $senderStmt->get_result()->fetch_assoc();

    $receiverStmt = $con->prepare("SELECT Name, ConfirmEmail FROM register WHERE MatriID = ?");
    $receiverStmt->bind_param("s", $receiverId);
    ...
    $fcmStmt = $con->prepare("
        ...
        LIMIT 1
    ");
    $fcmStmt->bind_param("s", $receiverId);
    $fcmStmt->execute();
    ...
    if (!empty($targetToken)) {
        ...
        $senderName = explode(" ", $senderData['Name'])[0];
  //  print_r($senderName); exit;
        $notificationMessage = "You have received an express interest from $senderName.";
        ...
        // Optional: Save push notification in DB
        saveNotification($con, [
            'notification_title' => 'New Interest Received 💖',
            'notification_text'  => $notificationMessage,
            'notification_type'  => 'INTEREST_NOTIFICATION',
            'notification_profile_id' => $receiverId, // receiver
            'notification_matching_id' => $matriId    // sender
        ]);
    }

    /**
     * STEP 5: Check SMTP and email verification
     */
    ...
    $emailStmt->bind_param("s", $receiverId);
    $emailStmt->execute();
    ...
    /**
     * STEP 6: Send email using mail() if SMTP is enabled and email is verified
     */
    if ((int)$on_off === 1 && $emailVerified === 'Yes') {
        $height = get_height($senderData['Height']);
        $senderName = explode(" ", $senderData['Name'])[0];
        ...
        $subject = "Interest Received by Someone";
        $message = "<!doctype html>
            <html>
            <head>
                <meta charset='utf-8'>
                <title>Interest Received by Someone</title>
            </head>
            <body>
                <table width='467' border='0' style='font-family:\"Lucida Grande\", \"Lucida Sans Unicode\", \"Lucida Sans\", \"DejaVu Sans\", Verdana, sans-serif' cellpadding='0' cellspacing='0'>
                    <tr>
                        ...
                        <td colspan='2' align='center' valign='middle'>Date: {$eisentdt}</td>
                    </tr>
                    <tr>
                        <td colspan='3'>
                            Dear {$receiverData['Name']},<br>
                            Interest Received by Someone: {$senderName}
                        </td>
                    </tr>
                    <tr>
                        <td><img src='https://www.dishavadhuvar.com/gallary/{$senderData['Photo1']}' width='209' height='232' alt=''/></td>
                        <td width='221' valign='top'>
                            <p>Name: {$senderName}</p>
                            <p>Age: {$senderData['Age']} Years</p>
                            <p>Height: {$height}</p>
                            <p>City: {$senderData['City']}</p>
                            <p>Occupation: {$senderData['Occupation']}</p>
                            <p>Education: {$senderData['Education']}</p>
                        </td>
                        <td width='24'>&nbsp;</td>
                    </tr>
                    <tr>
        ...
        if (mail($receiverData['ConfirmEmail'], $subject, $message, $headers)) {
