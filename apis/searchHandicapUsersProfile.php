...
try {
    ...
    // Check if logged user has handicap (spe_cases != 'None')
    if ($loggedUserData['spe_cases'] == 'None') {
        echo json_encode(["status" => "error", "message" => "Access denied: User not eligible for handicap search"]);
        exit;
    }
    ...
    $hadnicap_status = isset($_POST['handicap']) ? $_POST['handicap'] : 'Any';
    ...
    $handicaps = is_array($hadnicap_status) ? $hadnicap_status : [$hadnicap_status];
    ...
    // Add handicap filter (exclude None and empty)
    if ($hadnicap_status != 'Any' && !empty($hadnicap_status)) {
        $handicapStr = "'" . implode("','", array_map(function($hand) use ($con) {
            return mysqli_real_escape_string($con, $hand);
        }, $handicaps)) . "'";
        $sql .= " AND spe_cases IN ($handicapStr)";
    }
    $sql .= " AND spe_cases NOT IN ('None', '')";
    ...
    if ($hadnicap_status != 'Any' && !empty($hadnicap_status)) {
        $countSql .= " AND spe_cases IN ($handicapStr)";
    }
    $countSql .= " AND spe_cases NOT IN ('None', '')";
    ...
    if (mysqli_num_rows($rs_result) > 0) {
        while ($row = mysqli_fetch_array($rs_result)) {
            $profileData = [
                ...
                'spe_cases' => $row['spe_cases'] ?: 'Not Set',
            ...
            // Handle photo display logic (updated to match HTML)
        ...
        echo json_encode([
            "status" => "success",
            "message" => "Handicap search results found",
            "search_criteria" => [
                "gender" => $txtgender,
...
// Height conversion function (modified to return string instead of echo)
