...
/**
 * STEP 2: Count total viewed profiles excluding blocked ones
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND whom NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND whom NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
    $params = array_merge($params, $blockedProfiles);
    $types .= str_repeat("s", count($blockedProfiles));
}

$countSql = "SELECT COUNT(*) as total FROM profile_views 
             WHERE who = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch viewed profiles with pagination
 */
$sql = "SELECT whom, date 
        FROM profile_views 
        WHERE who = ? AND banstatus != '1' $whereNotIn 
        ORDER BY date DESC 
        LIMIT ?, ?";
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Viewed profiles fetched successfully" : "No viewed profiles found",
    "pagination" => [
        "current_page" => $page,
