...
/**
 * STEP 2: Count total ignored profiles excluding blocked ones
 */
$whereNotIn = "";
$params = [];
$types  = "s"; // for $matriId
...
$countSql = "SELECT COUNT(*) as total FROM `ignore` 
             WHERE matriid = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch ignored profiles with pagination
 */
...
if (!empty($profiles)) {
    echo json_encode([
        "status" => "success",
        "message" => "Ignored profiles fetched successfully",
        "pagination" => [
            "current_page" => $page,
