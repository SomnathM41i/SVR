...
/**
 * STEP 2: Count total profiles that shortlisted me excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND mat_id NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND mat_id NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
    $params = array_merge($params, $blockedProfiles);
    $types .= str_repeat("s", count($blockedProfiles));
}

$countSql = "SELECT COUNT(*) as total 
             FROM shortlist_profile 
             WHERE profile_id = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch profiles that shortlisted me with pagination
 */
$sql = "SELECT mat_id, when1 
        FROM shortlist_profile 
        WHERE profile_id = ? AND banstatus != '1' $whereNotIn 
        ORDER BY when1 DESC 
        LIMIT ?, ?";
...
while ($row = $result->fetch_assoc()) {
    $shortlisterId = $row['mat_id'];

    // Fetch profile details of the user who shortlisted
    ...
    $pstmt->bind_param("s", $shortlisterId);
    $pstmt->execute();
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Profiles that shortlisted you fetched successfully" : "No one has shortlisted your profile yet",
    "pagination" => [
        "current_page" => $page,
