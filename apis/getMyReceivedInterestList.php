...
/**
 * STEP 2: Count total pending express interest requests excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND eisender NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND eisender NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
...
$countSql = "SELECT COUNT(*) as total 
             ...
             WHERE eireceiver = ? AND status = 'Pending' AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch pending express interest requests with pagination
 */
$sql = "SELECT eisender, eisentdt 
        FROM expressinterest 
        WHERE eireceiver = ? AND status = 'Pending' AND banstatus != '1' $whereNotIn 
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($profileData) {
        $profiles[] = [
            ...
            "photo_url"      => "https://weddingsparampara.com/gallary/".$profileData['Photo1'],
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Received express interest requests fetched successfully" : "No pending express interest requests found",
    "pagination" => [
        "current_page" => $page,
