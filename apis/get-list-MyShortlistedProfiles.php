...
/**
 * STEP 2: Count total shortlisted profiles excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND profile_id NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND profile_id NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
...
$countSql = "SELECT COUNT(*) as total 
             ...
             WHERE mat_id = ? AND banstatus != '1' $whereNotIn";
...
/**
 * STEP 3: Fetch shortlisted profiles with pagination
 */
$sql = "SELECT profile_id, when1 
        FROM shortlist_profile 
        WHERE mat_id = ? AND banstatus != '1' $whereNotIn 
        ORDER BY when1 DESC 
        LIMIT ?, ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch profile details of the shortlisted user
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Shortlisted profiles fetched successfully" : "No profiles have been shortlisted yet",
    "pagination" => [
        "current_page" => $page,
