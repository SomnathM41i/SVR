...
/**
 * STEP 2: Count total "who viewed my profile" excluding blocked
 */
$whereNotIn = "";
$params = [$matriId];
$types  = "s";
...
if (!empty($blockedBy)) {
    $whereNotIn .= " AND who NOT IN (" . str_repeat('?,', count($blockedBy) - 1) . "?)";
    $params = array_merge($params, $blockedBy);
    $types .= str_repeat("s", count($blockedBy));
}

if (!empty($blockedProfiles)) {
    $whereNotIn .= " AND who NOT IN (" . str_repeat('?,', count($blockedProfiles) - 1) . "?)";
...
$countSql = "SELECT COUNT(*) as total 
             FROM profile_views 
             WHERE whom = ? AND banstatus != '1' $whereNotIn";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch profile details of the viewer
...
echo json_encode([
    "status" => "success",
    "message" => !empty($profiles) ? "Profiles who viewed me fetched successfully" : "No one has viewed your profile yet",
    "pagination" => [
        "current_page" => $page,
