...
$sql="select eisentdt,eireceiver from expressinterest where eisender='$login' and status='Pending' $exclSql and banstatus!='1' LIMIT $start_from, $limit";
$result1=mysqli_query($con,$sql);
$sql1="SELECT COUNT(*) FROM expressinterest where eisender='$login' And status='Pending' $exclSql and banstatus!='1'";
  ...
  <div class="mvv-container">
    <div class="mvv-activity-grid">
      ...
      while ($recs = mysqli_fetch_array($result1)) {
        $s=mysqli_query($con,"select * from register where MatriID='".$recs['eireceiver']."'");
        while($rec=mysqli_fetch_array($s)) {
          $is_block = mysqli_query($con,"select *from block_member where matriid ='$login' AND profile_id = '".$recs['eireceiver']."'");
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="interest_send?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="interest_send?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="interest_send?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
