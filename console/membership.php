...
$result = mysqli_query($con,"SELECT * from membershipplan order by planid");
	if(isset($_POST['submit']))
	{
		...
		$num=mysqli_num_rows($rs);
			...
			$q="insert into membershipplan(plandisplayname,plannoofcontacts,planduration,planamount,description1,description2,description3,description4,description5,description6,description7) values('$name','$contact','$duration','$amount','$description1','$description2','$description3','$description4','$description5','$description6','$description7')";
				...
				if($rs>0)
				{
					...
					header('location:membership?msg=newsuccess');
...
if(isset($_POST['Update']))
{
	...
	header('location:membership?msg=success');
                ...
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <h4 class="mt-3"><?php  echo $row['plandisplayname'];?></h4>
						 <h5 class="mt-3">Amount :<?php  echo $row['planamount'];?></h5>
                        <p><span class="badge bg-primary">No of Contacts:<?php  echo $row['plannoofcontacts'];?></span> </p>
                        <hr>
					

                       <button type="button" class="btn btn-icon btn-outline-primary me-2" data-bs-toggle="modal" data-bs-target="#modal-report2" data-id="<?php echo $row['planid'];?>"><i class="feather icon-edit-2"></i></button>
                       <?php if($row['plan_status']=='Active'){?>                     
					   <a href="membership_active?id=<?php  echo $row['planid'];?>&status=Inactive"><button type="button" class="btn btn-icon btn-outline-success"><i data-feather="check-circle"></i></button></a>
					   <?php } else { ?>
					   <a href="membership_active?id=<?php  echo $row['planid'];?>&status=Active"><button type="button" class="btn btn-icon btn-outline-danger"><i data-feather="slash"></i></button></a>
                       <?php } ?>
					</div>
                </div>
            </div>
				...
				<div aria-labelledby="swal2-title" aria-describedby="swal2-content" class="swal2-popup swal2-modal swal2-icon-success swal2-show" tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true" style="display: flex;">
					<div class="swal2-header">
						...
						<?php if($_GET['msg']=='success'){?>
							<h2 class="swal2-title" id="swal2-title" style="display: flex;"><?php echo "Membership Updated Succesfully" ?></h2>
						<?php } else if($_GET['msg']=='active') { ?>
							<h2 class="swal2-title" id="swal2-title" style="display: flex;"><?php echo "Membership Activated Succesfully" ?></h2>
    					<?php } else if($_GET['msg']=='inactive') { ?>
							<h2 class="swal2-title" id="swal2-title" style="display: flex;"><?php echo "Membership Inactivated Succesfully" ?></h2>
					...
					<div class="swal2-actions">
						<a href="membership" data-target="#" class="swal2-confirm swal2-styled"  style="display: inline-block;">OK</a>
      				</div>
    			</div> 
			</div>
		</div>   
