...
if(isset($_POST['Update']))
	{
		...
		$q="select * from e_city where city='$city' and dist_ref='$dist' and taluka_ref='$taluka' and id<>'$id'";
		$rs=mysqli_query($con,$q);
		$num=mysqli_num_rows($rs);
		if($num>0)
		{
...
$q1="select * from e_country";
$rs1=mysqli_query($con,$q1);
$num1=mysqli_num_rows($rs1);
			
$q2="select * from e_state ORDER BY state ASC";
$rs2=mysqli_query($con,$q2);
$num2=mysqli_num_rows($rs2);

$q3="select * from e_dist ORDER BY dist ASC";
...
xmlhttp.open("GET","get_city?id="+id,true);
xmlhttp.send();
}
                    ...
                    <div class="card-body">
                        <div class="row align-items-center m-l-0">
                            ...
                            <div class="col-sm-6 text-end">
                                <button class="btn btn-success btn-sm mb-3 btn-round" data-bs-toggle="modal" data-bs-target="#modal-report" data-id="1"><i class="feather icon-plus"></i> Add City</button>
                            </div>
							<div class="col-sm-12 ">
						    <form  name="country" id="country" method="post" action="add_dist.php">
							 <select  class="mb-3 form-control"  name="country" type="select" id="country"  required="required" onChange="fillstate(this.value);">
                                    <div align="" class=""> <?php  echo $msg; 
									     $_SESSION['country']=$_POST['country'];
									     $_SESSION['state']=$_POST['state'];
											$_SESSION['dist']=$_POST['dist'];?> </div>
                              ...
                              <?php   } else { ?>
								...
								<?php  
								  ...
								  <select  class="mb-3 form-control"  name="state" type="select" id="state"  required="required" onChange="filldist(this.value);">
										...
										while($data2=mysqli_fetch_assoc($ry) and $i<$num2)
													  {
														  ...
														  ?>
															  <option value="<?php  echo $data2['state']; ?>"><?php  echo $data2['state']; ?></option>
															  <?php  
														  $i++;
													  }
													 ?>
                               ...
                               <?php
								    ...
								    <select  class="mb-3 form-control"  name="dist" type="select" id="dist"  required="required" onChange="get_city(this.value);">
                                    
				                     <?php   if(isset($_POST['state'])) { ?>
									 <option value="<?php echo $_SESSION['dist']; ?>"><?php  echo $_SESSION['dist'];?></option>
						                 
										   <?php $ryy=mysqli_query($con,"select * from e_dist where sid2='".$_SESSION['state']."'");
											$i=0;
										   while($data3=mysqli_fetch_assoc($ryy) and $i<$num3){  ?>
										 
																		  
																<option value="<?php  echo $data3['dist']; ?>"><?php  echo $data3['dist']; ?></option>
															  <?php  
														  $i++;
													  }
													 ?>
                                      ...
                                      <?php
										...
										$i=0;
										while($data3=mysqli_fetch_assoc($rs3) and $i<$num3)
													  { ?>
															  <option value="<?php  echo $data3['dist']; ?>"><?php  echo $data3['dist']; ?></option>
															  <?php  
														  $i++;
            ...
            <div class="modal-body modalb">      
				<div aria-labelledby="swal2-title" aria-describedby="swal2-content" class="swal2-popup swal2-modal swal2-icon-success swal2-show" tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true" style="display: flex;">
					...
					<div class="swal2-actions">
						<a href="add_city" data-target="#" class="swal2-confirm swal2-styled"  style="display: inline-block;">OK</a>
      				</div>
    			</div> 
			</div>
		</div>   
				...
				<div aria-labelledby="swal2-title" aria-describedby="swal2-content" class="swal2-popup swal2-modal swal2-icon-success swal2-show" tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true" style="display: flex;">
					<div class="swal2-header">
    					...
    					<?php
      						...
      						{
    					?>
	  					<h2 class="swal2-title" id="swal2-title" style="display: flex;">City Inactivated Successfully</h2>
    					<?php
      						}
      						else
      						{
    					?>
    					<h2 class="swal2-title" id="swal2-title" style="display: flex;">City Activated Successfully</h2>
    					<?php
      						}
    					?>
  					</div>
