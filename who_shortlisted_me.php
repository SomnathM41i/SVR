...
<title>Who Shortlisted Me</title>
...
<section class="mvv-page-hero">
  <div class="mvv-container">
    ...
    <h1>Who Shortlisted Me</h1>
    <p>तुम्हाला शॉर्टलिस्ट कोणी केले ते पहा</p>
    <nav class="mvv-breadcrumb">
      <a href="index_dashboard">Home</a>
      <span>Who Shortlisted Me</span>
    </nav>
  </div>
</section>

<?php
$exclSql = $excl ? " AND mat_id NOT IN($excl)" : '';
$sqlview = mysqli_query($con,"SELECT * FROM shortlist_profile where profile_id='$login' $exclSql and banstatus!='1' LIMIT $start_from, $limit");
$sql1="SELECT COUNT(*) FROM shortlist_profile where profile_id='$login' $exclSql and banstatus!='1'";
  ...
  <div class="mvv-container">
    <div class="mvv-activity-grid">
      ...
      while($rowview=mysqli_fetch_array($sqlview)) {
        ...
        $is_block = mysqli_query($con,"select *from block_member where matriid='$login' AND profile_id='".$rowview['mat_id']."'");
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="who_shortlisted_me?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="who_shortlisted_me?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="who_shortlisted_me?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
...
<section class="mvv-section">
  <div class="mvv-container">
    <div style="text-align:center;padding:80px 20px;">
      ...
      <p style="color:#999;">No one has shortlisted you yet.</p>
