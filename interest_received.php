...
<section class="mvv-page-hero">
  <div class="mvv-container">
    ...
    <p>तुम्हाला मिळालेल्या आवडी पहा</p>
...
<?php
$exclSql = $excl ? " AND eisender NOT IN($excl)" : '';
$sql="select eisentdt,eisender from expressinterest where eireceiver='$login' and status='Pending' $exclSql and banstatus!='1' LIMIT $start_from, $limit";
$result1=mysqli_query($con,$sql);
$sql1="SELECT COUNT(*) FROM expressinterest where eireceiver='$login' and status='Pending' $exclSql and banstatus!='1'";
  ...
  <div class="mvv-container">
    <div class="mvv-activity-grid">
      ...
      while ($recs = mysqli_fetch_array($result1)) {
        $s=mysqli_query($con,"select * from register where MatriID='".$recs['eisender']."'");
        while($rec=mysqli_fetch_array($s)) {
          $is_block = mysqli_query($con,"select *from block_member where matriid ='$login' AND profile_id = '".$recs['eisender']."'");
      ...
      <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12">
        <div class="mvv-match-card">
          ...
          <div class="mvv-card-body">
            ...
            <div style="font-size:0.8rem;color:var(--mvv-muted);margin-top:4px;">Date: <?php echo $recs['eisentdt'];?></div>
          </div>
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="interest_received?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="interest_received?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="interest_received?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
