...
<select class="selectpicker" data-live-search="true" title="Select Religion" name="religion[]" id="religion" multiple  onChange="fillcaste(this.value)"  tabindex="5">
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 form-group">
<select class="selectpicker" data-live-search="true" title="Select Specical Case" name="handicap[]" id="handicap" multiple  tabindex="6" required >
                <option value="Any">Select Specical Case</option>
                <?php 
                  $rrs=mysqli_query($con,"SELECT * FROM special_case WHERE case_type != 'None' ");
                  while($rrow=mysqli_fetch_array($rrs))
                  {
                ?>
                  <option value="<?php echo $rrow['case_type'];?>"><?php echo $rrow['case_type'];?></option>
                <?php 
                    }
                  ?>
                 </select>
...
<!--           <a href="#dialog_send_message" data-toggle="modal">
          <button class="theme-btn btn btn-style-three mt-3" type="Submit" name=""  ><span class="btn-title">Save Search</span></button></a> -->
          <?php } else { ?>
...
function getsearch1()
{
  document.form1.action="handicap_search_result?page=1";
}
function getsearch2()
{
...
.bootstrap-select > .dropdown-toggle { position:relative; width:100%; z-index:1; text-align:right; white-space:nowrap; height:3rem; line-height:34px; border-color:#54d04c; }
.contact-form .form-group select { position:relative; width:100%; max-width:100%; z-index:1; text-align:right; margin:0; white-space:nowrap; height:3rem; line-height:34px; border-color:#54d04c; }
