...
$sqlview = mysqli_query($con,"SELECT * FROM expressinterest where eireceiver='$login' And status='Accept' $exclSql and banstatus!='1' LIMIT $start_from, $limit");
$sql1="SELECT COUNT(*) FROM expressinterest where eireceiver='$login' $exclSql and banstatus!='1'";
  ...
  <div class="mvv-container">
    <div class="mvv-activity-grid">
      ...
      while($rowview=mysqli_fetch_array($sqlview)) {
        $whom=$rowview['eisender'];
        $sqldata=mysqli_query($con,"select * from register where matriid='$whom'");
        $rec=mysqli_fetch_array($sqldata);
        $is_block = mysqli_query($con,"select *from block_member where matriid ='$login' AND profile_id = '".$rowview['eisender']."'");
      ...
      <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12">
        <div class="mvv-match-card">
          ...
          <div class="mvv-card-body">
            ...
            <div style="font-size:0.8rem;color:var(--mvv-muted);margin-top:4px;">Date: <?php echo $rowview['eisentdt'];?></div>
          </div>
    ...
    <div style="text-align:center;margin-top:30px;">
      <ul class="mvv-pagination">
        ...
        <?php if($page>1){ ?><li><a href="who_connected_me?page=<?php echo ($page-1)?>"><i class="bi bi-chevron-left"></i></a></li><?php } ?>
        <?php for($i=1;$i<=$total_pages;$i++){ ?>
          <li<?php echo ($i==$page)?' class="active"':''?>>
            <?php if($i==$page){ ?><span><?php echo $i?></span><?php }else{ ?><a href="who_connected_me?page=<?php echo $i?>"><?php echo $i?></a><?php } ?>
          </li>
        <?php } ?>
        <?php if($page<$total_pages){ ?><li><a href="who_connected_me?page=<?php echo ($page+1)?>"><i class="bi bi-chevron-right"></i></a></li><?php } ?>
      </ul>
    </div>
    <?php } ?>
  </div>
  ...
  <section class="mvv-section">
    <div class="mvv-container">
      <div style="text-align:center;padding:80px 20px;">
        ...
        <p style="color:#999;">No one has connected you yet.</p>
