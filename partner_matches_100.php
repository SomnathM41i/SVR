...
<?php foreach($pageMatches as $profile){ $encoded=urlencode(base64_encode($profile['MatriID'])); $photo='images/nophoto.jpg'; if(($profile['photo_visibility']??'')==='allphoto' && ($profile['Photo1Approve']??'')==='Yes' && !empty($profile['Photo1']) && $profile['Photo1']!=='nophoto.jpg') $photo='photoprocess.php?image=gallary/'.$profile['Photo1'].'&square=500'; $waMLoc=implode(', ', array_filter([$profile['City']??'', $profile['Taluka']??'', $profile['Dist']??''])); $waML=[]; $waML[]="\u{1F496} Check out this Matrimony Profile!"; $waML[]=''; $waML[]="\u{1F194} Profile ID: {$profile['MatriID']}"; $waML[]="\u{1F382} Age: ".(int)$profile['Age'].' years'; if(!empty($profile['Education'])) $waML[]="\u{1F393} Education: {$profile['Education']}"; if(!empty($profile['Occupation'])) $waML[]="\u{1F4BC} Occupation: {$profile['Occupation']}"; if(!empty($heightMap[(int)$profile['Height']])) $waML[]="\u{1F4CF} Height: {$heightMap[(int)$profile['Height']]}"; if(!empty($waMLoc)) $waML[]="\u{1F4CD} Location: $waMLoc"; if(!empty($profile['Maritalstatus'])) $waML[]="\u{1F48D} Marital Status: {$profile['Maritalstatus']}"; $waML[]=''; $waML[]="\u{1F517} View Full Profile:"; $waML[]=$baseUrl.'public_profile?id='.$encoded; $waML[]=''; $waML[]="Find your perfect life partner today \u{2764}\u{FE0F}"; $waUrl='https://api.whatsapp.com/send?text='.rawurlencode(implode("\n", $waML)); ?>
