...
$searchid=$_GET['id'];
$qry1=mysqli_query($con,"select * from notification where noti_sender='$strid' AND noti_receiver='$searchid' and notification_type='Shortlisted'");
...
{
mysqli_query($con,"insert into notification(noti_sender,noti_receiver,notification_type,notification_desc,seen,date_time)values('$strid','$searchid','Shortlisted','Shortlisted Profile','unseen',NOW())");
...
$shortview=mysqli_query($con,"INSERT INTO shortlist_profile(mat_id,profile_id,when1) VALUE('$strid','$searchid','$now')")or svr_db_fail($con);
	...
	$sql=mysqli_query($con,"select * from  register WHERE MatriID='$searchid'");
	$row=mysqli_fetch_array($sql);
	$nm=$row['Name'];


	$sql2=mysqli_query($con,"select * from  register WHERE MatriID='$strid'");
	...
	{
	include('smtp2.php');
	$mail->Subject="Shortlisted by Someone";
	$mail->Body="<!doctype html>
<html>
<head>
<meta charset='utf-8'>
<title>Shortlisted by Someone</title>
...
<table width='467' border='0' style='font-family:'Lucida Grande', 'Lucida Sans Unicode', 'Lucida Sans', 'DejaVu Sans', Verdana, sans-serif' cellpadding='0' cellspacing='0'>
  ...
  <tr>
    ...
    <td colspan='3'>
	Dear  ".$nm." ,<br>
	Your profile shorlisted by: ".$sendernm[0]." </td>
  </tr>
  <tr>
    <td colspan='3'>-------------------------------------------------------------------------------</td>
  </tr>
  ...
  <tr>
    <td>ID:<a href='https://www.weddingsparampara.com/full_profile?id=".$strid."'> ".$strid." View Profile</a></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
</table>
